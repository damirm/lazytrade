...
- Задержки в скорости выставления заявок пропорциональны количеству сделок, которые проводятся на аккаунте в течение торгового дня. 
Если вы выставляете много поручений (десятки тысяч в сутки) и испытываете проблемы со скоростью исполнения, рекомендуем 
использовать [асинхронный метод выставления заявок](/invest/services/orders/async) или открыть дополнительные брокерские счета и равномерно разделить торговлю между ними.
...
1. Сообщение в `OrderStateStream` и обновление в `GetOrders / GetOrderState`.
2. Сообщение в `PositionsStream` и обновление в`GetPositions`.
3. Сообщение в `PortfolioStream`, `TradeStream`, `OperationsStream` и обновление в `GetPortfolio`.
4. Операции в `GetOperationsByCoursor`.
...
- Обновления в `MarketDataStream` приходят немного быстрее, чем в `GetCandles`.

- В потоке котировок свечей есть ограничение на частоту отправки — свечи и стаканы отправляются не раньше, чем через 100мс с момента отправки последнего сообщения по тому же инструменту.


## Асинхронный метод выставления заявок

С целью уменьшения задержек доставки торговых поручений до биржи в T-Invest API есть 
[асинхронный метод выставления заявок](/invest/services/orders/async) — он позволяет ускорить выставление поручений на биржу.