...
- расписания торговых сессий;
- календарь выплат купонов по облигациям;
- размер гарантийного обеспечения по фьючерсам;
- дивиденды по ценной бумаге.

Протестировать и отправить запросы можно через REST-методы.

<Button
size="s"
href="/invest/api/instruments-service"
>
REST-методы
</Button>
...
### BondBy
Получить облигацию по ее идентификатору.
...
### Indicatives
Индикативные инструменты — индексы, товары и другие.
...
### DfaBy
Получить цифровой актив по его идентификатору.
...
### GetDividends
События выплаты дивидендов по инструменту.
...
### GetAssets
Список активов. Метод работает для всех инструментов, кроме срочных — фьючерсов и опционов.
...
### GetFavoriteGroups
Список групп избранных инструментов.
...
### GetAssetReports
Расписание выхода отчетностей эмитентов.
...
Мнения аналитиков по инструменту.
...
| Field | Type | Description |
| ----- | ---- | ----------- |
| id_type |  [InstrumentIdType](#instrumentidtype) | Тип идентификатора инструмента — `figi`, `ticker`. [Подробнее об идентификации инструментов](/invest/intro/intro/faq_identification/). |
...
| required_tests | Массив объектов [string](#string) | Тесты, которые необходимо пройти клиенту, чтобы совершать сделки по инструменту. Параметр учитывает пройденные тесты и отображает только недостающие. |
...
| lot |  [int32](#int32) | Лотность инструмента. Возможно совершение операций только на количества ценной бумаги, кратные параметру `lot`. [Подробнее](/invest/intro/intro/glossary). |
...
| sector |  [string](#string) | Сектор экономики. |
| issue_kind |  [string](#string) | Форма выпуска: <br/>**documentary** — документарная; <br/>**non_documentary** — бездокументарная. |
...
| focus_type |  [string](#string) : <br/>**equity** — акции;<br/>**fixed_income** — облигации;<br/>**mixed_allocation** — смешанный;<br/>**money_market** — денежный рынок;<br/>**real_estate** — недвижимость;<br/>**commodity** — товары;<br/>**specialty** — специальный;<br/>**private_equity** — private equity;<br/>**alternative_investment** — альтернативные инвестиции. |
...
| futures_type |  [string](#string) | Тип фьючерса: <br/>**physical_delivery** — физические поставки; <br/>**cash_settlement** — денежный эквивалент. |
| asset_type |  [string](#string) | Тип актива: <br/>**commodity** — товар; <br/>**currency** — валюта; <br/>**security** — ценная бумага; <br/>**index** — индекс. |
| basic_asset |  [string](#string) | Основной актив. |
| basic_asset_size |  [Quotation](#quotation) | Размер основного актива. |
| country_of_risk |  [string](#string) | Код страны риска — то есть страны, в которой компания ведет основной бизнес. |
| country_of_risk_name |  [string](#string) | Наименование страны риска — то есть страны, в которой компания ведет основной бизнес. |
...
| share_type |  [ShareType](#sharetype) | Тип акции — `[ShareType](/invest/services/instruments/methods#sharetype)`. |
| min_price_increment |  [Quotation](#quotation) | Шаг цены. |
| api_trade_available_flag |  [bool](#bool) | Возможность торговать инструментом через API. |
| uid |  [string](#string) | Уникальный идентификатор инструмента. |
| real_exchange |  [RealExchange](#realexchange) | Реальная площадка исполнения расчетов (биржа). |
...
| dividend_type |  [string](#string) | Тип выплаты: `Regular Cash` – регулярные выплаты, `Cancelled` – выплата отменена, `Daily Accrual` – ежедневное начисление, `Return of Capital` – возврат капитала, прочие типы выплат. |
| record_date |  [google.protobuf.Timestamp](#googleprotobuftimestamp) | Дата фиксации реестра по UTC. |
| regularity |  [string](#string) | Регулярность выплаты: `Annual` – ежегодная, `Semi-Anl` – каждые полгода, прочие типы выплат. |
...
| Field | Type | Description |
| ----- | ---- | ----------- |
| group_name |  [string](#string) | Название группы, не больше 255 символов. |
| group_color |  [string](#string) | Цвет группы. Принимает значения от `000000` до `FFFFFF` в HEX-формате. |
...
| Field | Type | Description |
| ----- | ---- | ----------- |
| instrument_id | Массив объектов [string](#string) | Массив идентификаторов инструментов. Принимает значение `figi` или `instrument_uid`. Если в группе будет хотя бы один из инструментов массива, в ответе у группы вернется признак `containsInstrument = true`. |
| excluded_group_id | Массив объектов [string](#string) | Массив идентификаторов групп, которые нужно исключить из ответа. |
...
| Field | Type | Description |
| ----- | ---- | ----------- |
| assets | Массив объектов [string](#string) | Массив идентификаторов активов, не больше 100 шт. |
...
| Field | Type | Description |
| ----- | ---- | ----------- |
| trade_id |  [int64](#int64) | Идентификатор сделки, уникален только в рамках ответа. |
...
| investor_position |  [string](#string) | Отношение покупателя/продавца к эмитенту. |
| percentage |  [float](#float) | Купленный/проданный объём от общего количества ценных бумаг на рынке. |
| is_option_execution |  [bool](#bool) | Признак того, является ли сделка реализацией опциона. |
...
| Field | Type | Description |
| ----- | ---- | ----------- |
| instrument_id | Массив объектов [string](#string) | Идентификаторы инструментов. Принимают значение `figi`, `instrument_uid` или `ticker + '_' + class_code`. |
...
| INSTRUMENT_ID_TYPE_ID | 5 | Универсальный тип идентификатора инструмента. Принимает значение `figi`, `instrument_uid` или `ticker + '_' + class_code`.|
