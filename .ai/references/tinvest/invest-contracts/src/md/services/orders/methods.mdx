...
### PostOrderAsync
Выставить заявку асинхронным методом.
...
| aci_value |  [MoneyValue](#moneyvalue) | Значение НКД (накопленного купонного дохода) на дату. [НКД при выставлении торговых поручений](/invest/services/orders/head-orders) |
| figi |  [string](#string) | Figi-идентификатор инструмента. |
| direction |  [OrderDirection](#orderdirection) | Направление сделки. |
...
| response_metadata |  [ResponseMetadata](#responsemetadata) | Метаданные. |
...
| executed_order_price |  [MoneyValue](#moneyvalue) | Цена исполнения заявки |
...
| Field | Type | Description |
| ----- | ---- | ----------- |
| [tracking_id](/invest/intro/developer/protocols/grpc/#tracking-id) |  [string](#string) | Идентификатор трекинга |
...
[Подробнее про конвертацию пунктов в валюту](/invest/intro/useful-info/points)
...
Для всех заявок и котировок на бирже устанавливается [шаг цены](/invest/services/quotes/faq_marketdata/#2.1). В случае использовании чисел с плавающей точкой необходимо проверять, что переданное в запросе значение соответствует шагу цены инструмента.
