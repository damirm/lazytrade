...
<Button
size="s"
href="/invest/api/operations-service"
>
REST-методы
</Button>
...
Список операций по счету. При работе с методом учитывайте
[особенности взаимодействия](/invest/services/operations/operations_problems).
...
Список операций по счету с пагинацией. При работе с методом учитывайте [особенности взаимодействия](/invest/services/operations/operations_problems).
...
### PortfolioStream
Server-side стрим обновлений портфеля.
...
### PositionsStream
Server-side стрим обновлений информации по изменению позиций портфеля.
...
| instrument_type |  [string](#string) | Тип инструмента: <br/><br/>`bond` — облигация; <br/>`share` — акция; <br/>`currency` — валюта; <br/>`etf` — фонд; <br/>`futures` — фьючерс. |
...
| state |  [OperationState](#operationstate) | Статус запрашиваемых операций указаны в `OperationState`. |
